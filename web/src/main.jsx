  ...
  return <>
    <div className="app-shell">
      <aside className={`sidebar ${mobileNavOpen ? 'mobile-open' : ''}`}>
        <div className="sidebar-header"><div className="brand"><span className="brand-mark"><CircleDollarSign size={21} /></span><span>ledger<span className="brand-dot">.</span></span></div><button className="mobile-close" aria-label="Close menu" onClick={() => setMobileNavOpen(false)}>ESC</button></div>
        <div className="sidebar-label">Your spaces</div>
        <div className="group-list">
          {groups.map((group) => <button className={`group-item ${selectedGroup?.id === group.id ? 'active' : ''}`} key={group.id} onClick={() => { setSelectedGroup(group); setMobileNavOpen(false) }}><span className="group-icon">{initials(group.name)}</span><span>{group.name}</span>{selectedGroup?.id === group.id && <ChevronDown size={15} />}</button>)}
          <button className="new-space" onClick={() => { setModal('group'); setMobileNavOpen(false) }}><Plus size={16} /> New space</button>
        </div>
  ...
  async function submit(event) {
    ...
    try {
      if (mode === 'forgot') {
        ...
        const payload = { email: form.email.trim(), password: form.password, ...(mode === 'register' ? { display_name: form.display_name.trim() } : {}) }
        const result = await request(`/v1/auth/${mode === 'login' ? 'login' : 'register'}`, { method: 'POST', body: JSON.stringify(payload) })
        onSignedIn(result.session_token)
      }
    } catch (err) { setError(err.message) } finally { setWorking(false) }
  }

  return <div className="auth-page"><div className="auth-art"><div className="brand light"><span className="brand-mark"><CircleDollarSign size={21} /></span><span>ledger<span className="brand-dot">.</span></span></div><div className="art-copy"><span className="eyebrow light-text">A better way to split</span><h1>Keep the good times.<br /><em>Lose the math.</em></h1><p>A shared ledger for the dinners, trips, and little moments that are better together.</p></div><div className="art-footer"><span>01</span><div className="art-line" /><span>MAKE IT EVEN</span></div></div><div className="auth-card"><div className="mobile-brand"><div className="brand"><span className="brand-mark"><CircleDollarSign size={21} /></span><span>ledger<span className="brand-dot">.</span></span></div></div><div className="auth-heading"><span className="eyebrow">{isReset ? 'Choose a new password' : mode === 'forgot' ? 'Account recovery' : 'Welcome back'}</span><h2>{isReset ? 'Make it secure again.' : mode === 'forgot' ? 'Find your way back.' : mode === 'login' ? 'Your shared life awaits.' : 'Start a shared space.'}</h2><p>{isReset ? 'Enter the token from your reset link and a new password.' : mode === 'forgot' ? 'Enter your email and we will create a reset link.' : mode === 'login' ? 'Sign in to pick up where you left off.' : 'Create your account and invite your people.'}</p></div><form onSubmit={submit}>{mode === 'register' && <label>Name<input required value={form.display_name} onChange={(e) => setForm({ ...form, display_name: e.target.value })} placeholder="What should we call you?" /></label>}{isReset && <label>Reset token<input required value={form.token} onChange={(e) => setForm({ ...form, token: e.target.value })} placeholder="Paste your reset token" /></label>}{mode !== 'reset' && <label>Email<input required type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} placeholder="you@example.com" /></label>}{mode !== 'forgot' && <label>Password<input required minLength="12" type="password" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} placeholder="12 characters minimum" /></label>}{error && <div className="form-error">{error}</div>}{notice && <div className="form-notice">{notice}</div>}<button className="primary-button wide" disabled={working}>{working ? 'Working...' : isReset ? 'Update password' : mode === 'forgot' ? 'Create reset link' : mode === 'login' ? 'Enter ledger' : 'Create account'} <ArrowUpRight size={17} /></button></form>{mode === 'login' && <button className="switch-auth" onClick={() => { setMode('forgot'); setError(''); setNotice('') }}>Forgot your password?</button>}{mode === 'forgot' && <button className="switch-auth" onClick={() => { setMode('reset'); setError(''); setNotice('') }}>I have a reset token</button>}<button className="switch-auth" onClick={() => { setMode(mode === 'login' || mode === 'forgot' || isReset ? 'register' : 'login'); setError(''); setNotice('') }}>{mode === 'register' ? 'Already have an account? Sign in' : 'New here? Create an account'}</button>{(mode === 'forgot' || isReset) && <button className="switch-auth" onClick={() => { setMode('login'); setError(''); setNotice('') }}>Back to sign in</button>}</div></div>
}
